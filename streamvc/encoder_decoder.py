...
class Encoder(nn.Module):
    def __init__(
        self, scale: int, embedding_dim: int, gradient_checkpointing: bool = False
    ):
        super().__init__()
...
class EncoderBlock(nn.Module):
    def __init__(
        ...
        self.block = nn.Sequential(
            ResidualUnit(
                in_channels, dilation=1, gradient_checkpointing=gradient_checkpointing
            ),
            ResidualUnit(
                in_channels, dilation=3, gradient_checkpointing=gradient_checkpointing
            ),
            ResidualUnit(
                in_channels, dilation=9, gradient_checkpointing=gradient_checkpointing
            ),
            CausalConv1d(
...
class DecoderBlock(nn.Sequential):
    def __init__(
        ...
        self.block = nn.Sequential(
            ...
            ResidualUnit(
                out_channels, dilation=1, gradient_checkpointing=gradient_checkpointing
            ),
            ResidualUnit(
                out_channels, dilation=3, gradient_checkpointing=gradient_checkpointing
            ),
            ResidualUnit(
                out_channels, dilation=9, gradient_checkpointing=gradient_checkpointing
            ),
        )
        self.gradient_checkpointing = gradient_checkpointing
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.unit = nn.Sequential(
            CausalConv1d(channels, channels, kernel_size, dilation=dilation, **kwargs),
            nn.ELU(),
