...
class StreamVC(nn.Module):
    def __init__(self, sample_rate: int = 16_000, gradient_checkpointing: bool = False):
        ...
        self.content_encoder = Encoder(
            scale=64, embedding_dim=64, gradient_checkpointing=gradient_checkpointing
        )
        self.speech_encoder = Encoder(
            scale=32, embedding_dim=64, gradient_checkpointing=gradient_checkpointing
        )
    ...
    @auto_batching(("* t", "* t"), "* t")
    def forward(
        self, source_speech: torch.Tensor, target_speech: torch.Tensor
    ) -> torch.Tensor:
        with torch.no_grad():
...
class _StreamingStreamVC:
    ...
    def forward(self, source_speech_chunck: torch.Tensor):
        ...
        energy = self.model.energy_estimator(source_speech_chunck).unsqueeze(dim=-1)
        source_linguistic_features = torch.cat([content_latent, f0, energy], dim=-1)
